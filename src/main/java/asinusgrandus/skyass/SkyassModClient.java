...
public class SkyassModClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		HudRenderCallback.EVENT.register(OnHudRender::onHudRender);
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			if (keyToggle.wasPressed()) {
				OnHudRender.switchHudEnabled();
				if (client.player != null) {
					client.player.sendMessage(OnHudRender.shouldRenderHud() ? Text.translatable("hud.elytra_flight_hud.hud_enabled") : Text.translatable("hud.elytra_flight_hud.hud_disabled"), true);
				}
			}
		});
